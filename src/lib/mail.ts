...
export async function sendWelcomeEmail(to: string, name: string, username: string, password: string) {
  ...
  const mailOptions = {
    from: `"ALL CRM — Gestión Inteligente" <${process.env.EMAIL_USER}>`,
    to,
    subject: 'Bienvenido al Equipo ALL CRM',
    html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #0B1623; color: #E2E8F0; padding: 40px; border-radius: 12px; max-width: 600px; margin: auto;">
        <div style="text-align: center; margin-bottom: 30px;">
          <h1 style="color: #00D1C8; margin: 0; font-size: 28px; letter-spacing: 2px;">ALL CRM</h1>
          <p style="color: #94A3B8; font-size: 12px; font-style: italic; margin-top: 5px;">Gestión Inteligente</p>
        </div>
        ...
        <div style="background-color: #132238; padding: 30px; border-radius: 16px; border: 1px solid rgba(56, 189, 248, 0.1); box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
          ...
          <p style="color: #94A3B8; line-height: 1.6;">Es un gusto darte la bienvenida al equipo de <b>ALL CRM</b>. Hemos creado tu acceso al sistema CRM para que puedas gestionar y dar seguimiento a nuestros proyectos.</p>
          
          <div style="background-color: #162133; padding: 20px; border-radius: 12px; margin: 25px 0; border: 1px solid rgba(56, 189, 248, 0.2);">
            <h3 style="color: #00D1C8; font-size: 14px; text-transform: uppercase; margin-top: 0; letter-spacing: 1px;">Tus Credenciales de Acceso</h3>
          ...
          <div style="text-align: center; margin-top: 35px;">
            <a href="${loginUrl}" style="background-color: #00D1C8; color: #0B1623; padding: 14px 30px; border-radius: 10px; text-decoration: none; font-weight: 800; font-size: 15px; display: inline-block; transition: all 0.3s ease;">
              ACCEDER AL CRM
            </a>
          </div>
        </div>
        ...
        <div style="text-align: center; margin-top: 30px; color: #64748B; font-size: 12px;">
          ...
          <p>© ${new Date().getFullYear()} ALL CRM. Todos los derechos reservados.</p>
        </div>
      </div>
    `,
  }
